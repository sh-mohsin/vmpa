...
class SongsOfAlbum extends StatefulWidget {
  final int albumId;
  final String albumTitle;
  const SongsOfAlbum({super.key, required this.albumId, required this.albumTitle});

  @override
  State<SongsOfAlbum> createState() => _SongsOfAlbumState();
}

class _SongsOfAlbumState extends State<SongsOfAlbum> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
      title: Text( widget.albumTitle ?? 'Album'),
    ),
      body: Column(
        children: [
          Expanded(child:
          albumSongs.isNotEmpty ? ListView.builder(
                      ...
                      Expanded(
                        child: SizedBox(
                          ...
                          child: Column(
                            ...
                            children: [
                              Text(albumSongs[index].title ?? '', overflow: TextOverflow.ellipsis,
                                maxLines: 2,),
                              Text(albumSongs[index].artist.toString()),
                            ],
                          ),
                        ),
                      ),
                        ...
                        child: Row(
                          ...
                          children: [
                            IconButton(onPressed: () async{
                              print('audioPath: ${albumSongs[index].uri}');
                              await Share.shareFiles([albumSongs[index].uri.toString()]);
    ...
    _audioQuery.querySongs(
      ...
      setState(() {
        ...
        for(var songs in audioList){
          if(songs.albumId == widget.albumId){
            albumSongs.add(SongModel({
              "artist_id" : songs.albumId,
              "artist" : songs.artist,
              "title" : songs.title,
              '_uri': songs.uri,
            }));
        ...
        print('artListLength: ${albumSongs.length}');
      });
    });
  }
